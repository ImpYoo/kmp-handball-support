...
class GetPhasesWithMatchesByTimestampUseCase {
    operator fun invoke(phases: List<Phase>, fromTimestamp: Long): List<Phase> {
        return phases.mapNotNull { phase ->
            val filteredMatchDays = phase.matchDays
                .map { matchDay ->
                    matchDay.copy(matches = matchDay.matches.filter { it.timestamp >= fromTimestamp })
                }
                .filter { it.matches.isNotEmpty() }

            if (filteredMatchDays.isEmpty()) {
                null
            } else {
                phase.copy(matchDays = filteredMatchDays)
            }
        }
    }
}
...
class GetMatchesOfPhaseByTimestampUseCase {
    operator fun invoke(phaseId: Int, phases: List<Phase>, fromTimestamp: Long): List<Match> {
        val phase = phases.firstOrNull { it.phaseId == phaseId } ?: return emptyList()
        return phase.matchDays.flatMap { it.matches }.filter { it.timestamp >= fromTimestamp }
    }
}
